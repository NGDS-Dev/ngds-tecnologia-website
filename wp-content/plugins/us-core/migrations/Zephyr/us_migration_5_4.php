...
class us_migration_5_4 extends US_Migration_Translator {
	...
	public function translate_us_logos( &$name, &$params, &$content ) {
		...
		if ( isset( $params['style'] ) ) {
			$params['items_layout'] = $this->get_logos_grid_layout( $params['style'] );
			unset( $params['style'] );
		} else {
			$params['items_layout'] = $this->get_logos_grid_layout( '1' );
		}
	...
	private function get_logos_grid_layout( $style ) {

		$logos_grid_layout = array(
			'title' => 'Logos Showcase Fade + Outline',
			'data' => array(
				'post_image:1' => array(
					'link' => 'custom',
					'custom_link' => array(
					...
					'color_bg_hover' => ( $style == 1 ) ? us_get_option( 'color_content_bg_alt' ) : '',
					'opacity_hover' => '1',
			...
			'default' => array(
				...
				'options' => array(
					'overflow' => TRUE,
					'border_radius' => ( $style == 1 ) ? '0.2rem' : '',
					'box_shadow_hover' => ( $style == 1 ) ? '0.5rem' : '',
				),
			),
		);
	...
	public function translate_grid_layout_settings( &$settings ) {
		...
		foreach ( $settings['data'] as $name => $data ) {
			...
			if ( ! empty( $data['text_styles'] ) AND is_array( $data['text_styles'] ) ) {
				...
				if ( isset( $settings['data'][ $name ]['text_styles'] ) ) {
		...
		foreach ( $settings['data'] as $name => $data ) {
			...
			if ( substr( $name, 0, 4 ) == 'menu' ) {
				...
				if ( ! empty( $data['text_style'] ) AND is_array( $data['text_style'] ) ) {
					...
					if ( isset( $settings['data'][ $name ]['text_style'] ) ) {
	...
	public function translate_theme_options( &$options ) {
		...
		if ( isset( $options['optimize_assets'] ) AND $options['optimize_assets'] AND is_array( $options['assets'] ) ) {
			$options['assets'] = array_unique( array_merge( $options['assets'], array( 'pagination', 'grid' ) ) );
		}
		...
		if ( $post_type == 'post' AND ( ! empty( $meta['us_post_preview_layout'][0] ) OR ! empty( $meta['us_sidebar'][0] ) OR $this->content_has_row ) ) {
			...
			if ( empty( $meta['us_content_id'][0] ) ) {
				...
				if ( $us_migration_doing_fallback ) {
					update_post_meta( $us_migration_current_post_id, 'us_content_id', $meta['us_content_id'][0] );
				}
			}
	...
	private function get_page_blocks() {
		if ( $this->page_blocks === NULL ) {
			$this->page_blocks = array();
			ob_start();
			$posts = get_posts( array( 'post_type' => 'us_page_block', 'numberposts' => - 1 ) );
			ob_end_clean();
			foreach ( $posts as $post ) {
				$this->page_blocks[ $post->ID ] = $post->post_content;
			}
		}
	}

	// Create Reusable Block for Posts based on former Posts settings
	private function add_post_content_template( $featured_image_layout = NULL, $has_sidebar = NULL ) {
		global $usof_options;
		usof_load_options_once();
		$this->get_page_blocks();
		...
		// Generate Reusable Block title
		$post_title = 'Blog Post with ' . $featured_image_layout . ' preview';

		// Generate Reusable Block content
		$post_content = '[vc_row';
		...
		// Author Box
		if ( $usof_options['post_author_box'] ) {
			$post_content .= '[us_post_author color_link="" avatar="1" avatar_pos="left" website="1" info="1" font_size="1.2rem" css=".vc_custom_11223345{padding: 2rem !important;background-color: ' . $usof_options['color_content_bg_alt'] . ' !important;}"]';
			$post_content .= '[us_separator size="small"]';
		} else {
			$post_content .= '[us_separator size="small" show_line="1"]';
		}
		// Prev/Next navigation
		if ( $usof_options['post_nav'] ) {
			$post_content .= '[us_post_navigation';
			$post_content .= ( $usof_options['post_nav_layout'] == 'sided' ) ? ' layout="sided"' : '';
			$post_content .= ( $usof_options['post_nav_invert'] ) ? ' invert="1"' : '';
			$post_content .= ( $usof_options['post_nav_category'] ) ? ' in_same_term="1"' : '';
			$post_content .= ']';
			$post_content .= ( $usof_options['post_nav_layout'] == 'sided' ) ? '' : '[us_separator size="small" show_line="1"]';
		}
		// Related Posts
		if ( $usof_options['post_related'] ) {
			$post_content .= '[vc_column_text]<h4>' . strip_tags( $usof_options['post_related_title'] ) . '</h4>[/vc_column_text]';
			$post_content .= '[us_separator size="small"]';
			$post_content .= '[us_grid post_type="related"';
			$post_content .= ( $usof_options['post_related_type'] == 'category' ) ? '' : ' related_taxonomy="post_tag"';
			$post_content .= ' orderby="' . $usof_options['post_related_orderby'] . '"';
			$post_content .= ' items_quantity="' . $usof_options['post_related_quantity'] . '"';
			$post_content .= ' items_layout="' . $usof_options['post_related_layout'] . '"';
			$post_content .= ' img_size="' . $usof_options['post_related_img_size'] . '"';
			$post_content .= ' columns="' . $usof_options['post_related_cols'] . '"';
			$post_content .= ' items_gap="' . $usof_options['post_related_items_gap'] . '"]';
			$post_content .= '[us_separator size="small" show_line="1"]';
		}
		// Comments
		...
		if ( $present_block_post_id = array_search( $post_content, $this->page_blocks ) ) {
			return $present_block_post_id;
		}

		// Modify Reusable Block title if has sidebar
		if ( $has_sidebar ) {
		...
		$this->page_blocks[ $block_post_id ] = $post_content;

		return $block_post_id;
	}

	// Create Reusable Block with params, based on former Archive Pages settings
	private function add_archive_content_template( $for_author_pages = FALSE ) {
		...
		if ( $for_author_pages ) {
			...
			$post_content .= '[us_post_author link="none" avatar="1" avatar_pos="left" posts_count="1" website="1" info="1" font_size="1.2rem" css=".vc_custom_1122334455{padding: 2rem !important;background-color: ' . $usof_options['color_content_bg_alt'] . ' !important;}"]';
		...
		$this->page_blocks[ $post_id ] = $post_content;
	...
	// Create Reusable Block for Portfolio Pages with Prev/Next navigation
	private function add_portfolio_content_template( $invert, $within_category ) {
		...
		$block_post_array = array(
			...
			'post_date' => date( 'Y-m-d H:i', time() - 86400 ),
			'post_status' => 'publish',
			'post_title' => 'Page Template: Portfolio with Prev/Next navigation',
			'post_content' => $post_content,
		);
		ob_start();
		$block_post_id = wp_insert_post( $block_post_array );
