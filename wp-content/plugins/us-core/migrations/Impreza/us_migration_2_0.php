...
class us_migration_2_0 extends US_Migration_Translator {
	...
	public function migration_completed_message() {
		...
		$output .= '<p><strong>Migration to ' . US_THEMENAME . ' ' . US_THEMEVERSION . ' is completed</strong>. Now please regenerate thumbnails and check your website once again. If you notice some issues, <a href="'. $help_portal_url .'/impreza/installation/update20/" target="_blank">follow the manual</a>.</p>';
		$output .= '</div>';
	...
	public function translate_theme_options( &$options ) {
		...
		foreach ( range( 2, 10 ) as $number ) {
			if ( isset( $options['header_language_' . $number . '_url'] ) ) {
				$options['header_language_' . $number . '_url'] = str_replace( '[site_url]', home_url(), $options['header_language_' . $number . '_url'] );
			}
		}
	...
	public function translate_vc_row( &$name, &$params, &$content ) {
		...
		if ( isset( $params['css'] ) AND ! empty( $params['css'] ) ) {
			...
			foreach ( $css_params as $k => $css_param ) {
				...
				if ( trim( (string) $css_param_info[0] ) == 'background-color' ) {
					$params['us_bg_color'] = trim( (string) $css_param_info[1] );
					unset( $css_params[ $k ] );
					$shortcode_changed = $css_params_changed = TRUE;
				}
				if ( in_array( trim( (string) $css_param_info[0] ), array( 'background-image', 'background' ) ) ) {
					if ( preg_match( '/\?id=(\d+)/i', trim( (string) $css_param_info[1] ), $bg_image_matches ) ) {
