...
if ( ! empty( $link_atts['href'] ) OR usb_is_post_preview() ) {
	if ( $link_type == 'container' ) {
		...
		if ( $btn_label !== '' ) {
			$link_atts['class'] = 'w-btn ' . us_get_btn_class( $btn_style );
			if ( ! empty( $btn_size ) ) {
				$link_atts['style'] = 'font-size:' . $btn_size;
			}

			$btn_html .= '<a' . us_implode_atts( $link_atts ) . '>';
			$btn_html .= '<span class="w-btn-label">' . $btn_label . '</span>';
			$btn_html .= '</a>';
		}
	}
}
...
if (
	$front_bgimage = us_replace_dynamic_value( $front_bgimage, /* acf_format */ FALSE )
	AND $front_bgimage_url = wp_get_attachment_image_url( $front_bgimage, $front_bgimage_size )
) {
...
if ( $front_icon_type == 'font' ) {
	...
	$front_icon_image = us_replace_dynamic_value( $front_icon_image, /* acf_format */ FALSE );
...
$output_front_title = '';
if ( ! empty( $front_title ) OR usb_is_post_preview() ) {
...
$output_front_desc = '';
if ( ! empty( $front_desc ) OR usb_is_post_preview() ) {
...
if ( ! us_amp() ) {
	...
	if (
		$back_bgimage = us_replace_dynamic_value( $back_bgimage, /* acf_format */ FALSE )
		AND $back_bgimage_url = wp_get_attachment_image_url( $back_bgimage, $front_bgimage_size )
	) {
	...
	if ( ! empty( $back_title ) OR usb_is_post_preview() ) {
	...
	if ( ! empty( $back_desc ) OR usb_is_post_preview() ) {
