...
if ( empty( $url ) ) {
	...
	$url .= str_replace( '?us_iframe=1', '', us_get_safe_var( 'REQUEST_URI' ) );
}
...
foreach ( $providers_list as $provider => $provider_data ) {
	...
	if ( us_amp() ) {
		if ( ! in_array( $provider, array( 'vk', 'xing', 'reddit' ) ) ) {
			...
			if ( $provider == 'pinterest' ) {
				...
				$amp_item_atts['data-param-app_id'] = trim( (string) us_get_option( 'facebook_app_id', '' ) );
			}
	...
	if ( $provider == 'email' ) {
		...
		$items_html .= us_prepare_icon_tag( 'fab|' . $provider );
		$tooltip_items_html .= us_prepare_icon_tag( 'fab|' . $provider );
	}
...
if ( $text_selection AND ! us_amp() AND $us_sharing_index === 1 ) {
	...
	$output .= '<button' . us_implode_atts( $item_atts ) . '>';
