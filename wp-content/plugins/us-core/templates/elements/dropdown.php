...
if ( $source == 'own' ) {
	...
	foreach ( $links as $link ) {
		$data['list'][] = array(
			'icon' => us_prepare_icon_tag( us_arr_path( $link, 'icon', '' ) ),
			'link_atts' => us_generate_link_atts( us_arr_path( $link, 'url', '' ) ),
			'title' => us_arr_path( $link, 'label', '' ),
		);
	}
...
if ( ! empty( $data['current'] ) ) {
	...
	if ( us_amp() ) {
		$current_atts['on'] = 'tap:' . $_atts['id'] . '.toggleClass(class=\'opened\')';
	} else {
		$current_atts['href'] = 'javascript:void(0)';
	}

	$output .= '<a' . us_implode_atts( $current_atts ) . '>';
