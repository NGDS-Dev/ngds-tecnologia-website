...
! function( $, undefined ) {
	...
	$usof.field[ 'transfer' ] = {
		...
		init: function() {
			...
			// Get extra options
			$extraOptions = $( '.usof-transfer-extra-options', this.$row );
			if ( $extraOptions.is( '[onclick]' ) ) {
				this.extraOptions = $extraOptions[ 0 ].onclick() || {};
				$extraOptions.remove();
			} else {
				this.extraOptions = {};
			}
		...
		exportValues: function() {
			var values = $.extend( this.hiddenFieldsValues, $usof.instance.getValues(), this.extraOptions );
