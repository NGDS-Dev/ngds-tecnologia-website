...
/** This filter is documented in https://developer.wordpress.org/reference/hooks/tiny_mce_before_init/ */
if ( $tinymce_settings = apply_filters( 'tiny_mce_before_init', array(), (string) $id ) ) {
	if ( ! isset( $editor_settings[ 'tinymce' ] ) ) {
		$editor_settings[ 'tinymce' ] = array();
	}
	$editor_settings[ 'tinymce' ] = array_merge( $editor_settings[ 'tinymce' ], $tinymce_settings );
}
