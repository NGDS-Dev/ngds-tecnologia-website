...
class US_Migration {
	...
	protected function __construct() {
		...
		if ( is_dir( $migrations_path ) ) {
			foreach ( glob( $migrations_path . '*.php' ) as $migration_file ) {
				$class = wp_basename( $migration_file, '.php' );
	...
	public static function get_latest_version_migration() {
		...
		if ( is_dir( $migrations_path ) ) {
			foreach ( glob( $migrations_path . '*.php' ) as $migration_file ) {
				if ( preg_match( '/^us_migration_([\d\_]+)$/', wp_basename( $migration_file, '.php' ), $matches ) ) {
	...
	public function display_migration_needed() {
		...
		if ( $this->migration_needed_message != '' ) {
			...
			if ( ! defined( 'PHP_VERSION_ID' ) OR PHP_VERSION_ID < 50600 ) {
				...
				$output .= '<a href="https://wordpress.org/support/update-php/" target="_blank">Learn more about updating PHP</a></p>';
				...
				$output .= '<p><label><input type="checkbox" name="allow_migration" id="allow_migration"> I did all the steps described in the <a href="https://help.us-themes.com/impreza/installation/migration/" target="_blank"><strong>instructions</strong></a></p>';
...
abstract class US_Migration_Translator {
	...
	protected function _translate_menus( &$locations, $rules ) {
		...
		foreach ( $rules as $old => $new ) {
			if (
				isset( $locations[ $old ] )
				AND in_array( $locations[ $old ], $menu_ids )
			) {
