			...
			foreach ( $params as $param => $param_values ) {
				...
				foreach ( $param_values as $item_value ) {
					if (
						...
						OR is_numeric( $item_value ) // Note: For numeric value ZERO
						OR ! empty( $item_value )
					) {
...
if ( ! function_exists( 'us_apply_grid_filters' ) ) {
	 ...
	 */
	function us_apply_grid_filters( &$query_args, $grid_filter_params = NULL ) {
		...
		// Get grid filter params
		$filter_ranges = array();
		$filter_taxonomies = us_get_filter_taxonomies( us_get_grid_url_prefix( 'filter' ), $grid_filter_params );
...
if ( ! function_exists( 'us_grid_set_orderby_to_query_args' ) ) {
	...
	function us_grid_set_orderby_to_query_args( &$query_args, $params = array() ) {
		...
		switch ( $params['orderby'] ) {
			...
			case 'custom':
				/**
				 * The `_orderby_custom_` value does not affect anything, it only allows you to preserve uniqueness
				 * @link https://dev.mysql.com/doc/refman/8.0/en/cast-functions.html#function_cast
				 */
				$orderby_custom = apply_filters( 'us_orderby_custom_for_grid', array(
					'relation' => 'OR',
					array(
						'compare' => 'EXISTS',
						'key' => $params['custom_field'],
						'type' => empty( $params['custom_field_numeric'] ) ? 'CHAR' : 'DECIMAL(10,3)',
					),
					array(
						'compare' => 'NOT EXISTS',
						'key' => $params['custom_field'],
				...
				$query_args['meta_query']['_orderby_custom_'] = $orderby_custom;
...
if ( ! function_exists( 'us_get_terms_for_autocomplete' ) ) {
	...
	/**
	 * Request AJAX handler for us_get_terms_for_autocomplete
	 * @return string
	 */
	function us_ajax_get_terms_for_autocomplete() {
		...
		if ( is_admin() ) {
			...
			$taxonomy = (string) us_arr_path( $_GET, 'taxonomy' );
