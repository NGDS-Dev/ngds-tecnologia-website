...
class US_Breadcrumbs {
	...
	public function __construct( $delimiter, $home, $item_before, $item_after, $link_attr, $name_attr, $position_attr ) {
		...
		$this->config['before_basic'] = '<div class="g-breadcrumbs-item">';
		// Code after the current crumb
		$this->config['after'] = isset( $item_after ) ? $item_after : '';
		$this->config['after_basic'] = '</div>';
		// Links attributes
		...
		$this->config['homeLink'] = ( has_filter( 'us_tr_home_url' ) )
	...
	public function render() {
		...
		// Note: The Events Calendar override the `$post` global variable, which contains the wrong object
		...
		if (
			...
			AND function_exists( 'tribe_is_upcoming' )
			AND function_exists( 'tribe_is_past' )
			AND function_exists( 'tribe_is_day' )
		) {
		...
		if ( is_home() ) {
			...
			// Portfolio Pages
			if ( $post_type == 'us_portfolio' AND us_get_option( 'portfolio_breadcrumbs_page' ) != '' ) {
				...
				if ( ! $taxonomies_found ) {
					$cpt_post_type = get_post_type_object( $post_type );
		...
		if ( get_query_var( 'paged' ) AND ! in_array( $post_type, array( 'topic', 'forum' ) ) ) {
			$pagination_in_taxonomy = ( is_category() OR is_day() OR is_month() OR is_year() OR is_search() OR is_tag() OR is_author() );

			if ( $pagination_in_taxonomy ) {
				$output .= ' (';
			} else {
				$output .= $this->config['delimiter'];
			}
