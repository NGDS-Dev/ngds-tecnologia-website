...
if ( ! function_exists( 'us_ajax_get_current_id' ) ) {
	add_filter( 'us_tr_object_id', 'us_ajax_get_current_id', 1, 1 );
	/**
	 ...
	 */
	function us_ajax_get_current_id( $current_id ) {
		global $us_page_args;
...
function us_ajax_grid() {
		...
		foreach ( $filters_args['taxonomies_query_args'] as &$items ) {
			foreach ( $items as &$item_query_args ) {
				...
				if ( ! is_null( $template_vars['us_grid_filter_params'] ) ) {
					us_apply_grid_filters( $item_query_args, $template_vars['us_grid_filter_params'] );
				}
	...
	if ( function_exists( 'us_wc_get_min_max_price' ) AND ! empty( $filters_args['wc_min_max_price'] ) ) {
		...
		if ( ! is_null( $template_vars['us_grid_filter_params'] ) ) {
			us_apply_grid_filters( $min_max_price_query_vars, $template_vars['us_grid_filter_params'] );
		}
	...
	if ( ! is_null( $template_vars['us_grid_filter_params'] ) ) {
		us_apply_grid_filters( $template_vars['query_args'], $template_vars['us_grid_filter_params'] );
	}
	...
	if (
		...
		function us_ajax_pre_get_posts( $query ) {
			...
			if ( ! $us_is_page_block_in_no_results AND method_exists( wc()->query, 'product_query' ) ) {
	...
	if ( ! function_exists( 'us_posts_orderby_for_search' ) AND empty( $grid_orderby ) ) {
		...
		function us_posts_orderby_for_search( $orderby, $wp_query ) {
			...
			if (
				...
				$order = esc_sql( $order );
				$orderby = "{$wpdb->posts}.post_title {$order}, {$wpdb->posts}.post_date {$order}";
			}
