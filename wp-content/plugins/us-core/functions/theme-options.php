...
function us_generate_asset_file( $ext, $custom_language_domain = '' ) {
	...
	if ( us_arr_path( $usof_options, 'optimize_assets', FALSE ) ) {
		...
		foreach ( $assets_config as $component => $component_atts ) {
			...
			if (
				! empty( $component_atts['include_if'] )
				OR ! isset( $usof_options['assets'] )
				OR ! isset( $usof_options['assets'][ $component ] )
				OR $usof_options['assets'][ $component ] == 1
			) {
				...
				// Move assets to the top of generated file if set
				if ( ! empty( $component_atts['include_first'] ) ) {
		...
		if ( $ext == 'css' ) {
			...
			if ( $usof_options['include_gfonts_css'] ) {
				...
				$gfont_file_url = us_enqueue_google_fonts( /* return url */TRUE );
				...
				if ( $gfont_file_url ) {
					...
					// First, trying to fetch file with cURL
					...
					// If previous method didn't work, trying to fetch file with file_get_contents
					if ( ! $gfont_file_copied AND function_exists( 'ini_get' ) AND ini_get('allow_url_fopen') ) {
						...
						if (
							$_font_url = us_enqueue_google_fonts( /* return url */TRUE )
							AND $gfont_file_contents = file_get_contents( $_font_url, FALSE, $context )
						) {
							$handle = fopen( $gfont_file_path, 'w' );
							fwrite( $handle, $gfont_file_contents );
							$gfont_file_copied = TRUE;
					...
					// If something is wrong with cURL and file_get_contents, trying to use copy function
				...
				foreach ( $icon_css_files as $fonticon_name => $filepath ) {
					...
					if (
						...
						foreach ( $used_icons as $icon ) {
							...
							// Skip "Duotone" icons styles, if they don't exist (in Zephyr theme) or if "Font Awesome Duotone" set is disabled
							if (
								$icon_set === 'fad'
								AND (
									! isset( $usof_options['icons_fad'] )
									OR $usof_options['icons_fad'] == 'none'
								)
							) {
								continue;
							}
		...
		if (
			(
				// First check if the file is not created yet ...
				! file_exists( $file )
				// ... or if it is created and writeable ...
				OR (
					is_writable( $file )
					AND is_file( $file )
				)
			)
			// ... then try try to open it ...
			AND $handle = fopen( $file, 'w' )
		) {
			// ... and try to write contents to it
		...
		update_option( 'us_theme_options_css', us_minify_css( us_get_template( 'templates/css-theme-options' ) ), /* autoload */TRUE );
	}
...
function us_maybe_flush_rewrite_rules( $updated_options ) {
	...
	if ( $flush_rules ) {
		update_option( 'us_flush_rules', TRUE, /* autoload */TRUE );
	}
}
...
if ( ! function_exists( 'us_replace_login_logo' ) ) {
	...
	function us_replace_login_logo() {
		...
		if ( us_get_option( 'white_label' ) AND $white_label_icon = us_get_option( 'white_label_login_icon' ) ) {
...
// Replace "wordpress.org" link on wp-login screen (when it has a custom icon)
if ( ! function_exists( 'us_replace_login_logo_url' ) ) { 
	if ( get_site_icon_url() ) {
		add_filter( 'login_headerurl', 'us_replace_login_logo_url' );
	}
	function us_replace_login_logo_url() {
		return home_url();
	}
}
...
function us_exclude_special_pages_from_search( $query ) {
	if ( $query->is_search && $query->is_main_query() ) {
		$special_pages = array();
		$special_pages_names = array( 'search_page', 'page_404' );
		...
		// Exclude Posts page
		if ( $posts_page = (int) get_option( 'page_for_posts' ) ) {
			$special_pages[] = $posts_page;
		}
