...
function us_woocommerce_support() {
	...
	if ( empty( $product_gallery_options ) ) {
		add_filter( 'woocommerce_single_product_image_thumbnail_html', 'us_woocommerce_single_product_image_thumbnail_html' );
	}
...
if ( ! function_exists( 'us_woocommerce_single_product_image_thumbnail_html' ) ) {
	function us_woocommerce_single_product_image_thumbnail_html( $html ) {
...
if ( ! function_exists( 'us_wc_get_min_max_price' ) ) {
	...
	function us_wc_get_min_max_price( $query_vars = array() ) {
		...
		$min_price = floor( (float) $prices->min_price );
		$max_price = ceil( (float) $prices->max_price );
...
if ( ! function_exists( 'us_checkout_page_content' ) ) {
	...
	function us_checkout_page_content( $content ) {
		...
		if (
			...
			if ( strpos( $content, 'class="w-wc-coupon-form is_checkout' ) !== FALSE ) {
