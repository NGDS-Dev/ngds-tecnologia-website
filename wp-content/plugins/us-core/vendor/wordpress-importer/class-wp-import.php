...
<?php if ( ! empty( $this->authors ) ) : ?>
	...
	<p>To make it simpler for you to edit and save the imported content, you may want to reassign the author of the imported item to an existing user of this site, such as your primary administrator account.</p>
...
<?php endif; ?>
	...
	function author_select( $n, $author ) {
		echo 'Import author: <strong>' . esc_html( $author['author_display_name'] );
		if ( '1.0' != $this->version ) {
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				printf( 'Failed to import %1$s %2$s', esc_html( $term['term_taxonomy'] ), esc_html( $term['term_name'] ) );
				if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
					echo ': ' . $id->get_error_message();
				}
				echo '<br />';
		...
		foreach ( $this->posts as $post ) {
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( '%1$s &#8220;%2$s&#8221; already exists.', $post_type_object->labels->singular_name, esc_html( $post['post_title'] ) );
				echo '<br />';
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							printf( 'Failed to import %1$s %2$s', esc_html( $taxonomy ), esc_html( $term['name'] ) );
							if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
	...
	function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			echo 'Menu item skipped due to missing menu slug<br />';
			return;
		}
	...
	function process_attachment( $post, $url ) {
		...
		if ( preg_match( '|^/[\w\W]+$|', $url ) ) {
			$url = rtrim( (string) $this->base_url, '/' ) . $url;
		}
