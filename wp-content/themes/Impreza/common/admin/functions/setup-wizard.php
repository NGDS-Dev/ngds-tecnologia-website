...
if ( ! function_exists( 'us_import_content_portfolio' ) ) {
	...
	function us_import_content_portfolio() {
		if (
			...
			us_update_option( 'enable_portfolio', /* set value */ 1 );
			us_create_portfolio_post_type();
		}
...
if ( ! function_exists( 'us_import_content_testimonials' ) ) {
	...
	function us_import_content_testimonials() {
		if (
			...
			us_update_option( 'enable_testimonials', /* set value */ 1 );
			us_create_testimonials_post_type();
		}
...
if ( ! function_exists( 'us_import_content_woocommerce' ) ) {
	...
	function us_import_content_woocommerce() {
		...
		if ( ! is_wp_error( $file_path ) ) {
			 ...
			 * @return string|null
			 */
			$func_get_page_id_by_title = function ( $page_title ) use ( $wpdb ) {
				return $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_title = '" . $page_title . "' AND post_type = 'page' AND post_status = 'publish'" );
			};

			// Set WooCommerce Pages
			if ( $shop_page_id = $func_get_page_id_by_title( 'Shop' ) ) {
				update_option( 'woocommerce_shop_page_id', $shop_page_id );
			}
			if ( $cart_page_id = $func_get_page_id_by_title( 'Cart' ) ) {
				update_option( 'woocommerce_cart_page_id', $cart_page_id );
			}
			if ( $checkout_page_id = $func_get_page_id_by_title( 'Checkout' ) ) {
				update_option( 'woocommerce_checkout_page_id', $checkout_page_id );
			}
			if ( $my_account_page_id = $func_get_page_id_by_title( 'My Account' ) ) {
